...
--[[Does not fire when object is locked]]--
--[[
...
function onPlayerAction(player, action, targets)
  if action == Player.Action.PickUp then
    onMouse(nil, nil, 1)
  else if action == Player.Action.Drop then
    onMouse(nil, nil, 0)
  end
end
]]--

--[[use scripting buttons as additional mouse events]]--
function onScriptingButtonDown(index, player)
  if index == 1 then onMouse(nil, nil, 1) end
end
function onScriptingButtonUp(index, player)
  if index == 1 then onMouse(nil, nil, 0) end
end
